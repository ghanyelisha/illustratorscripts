...
*/
    ...
    function startAction(){
        ...
        if (outerPath == null){alert ("Got a bad path. What's going on?");}
            ...
            function isPointInsideWithHoles(pt) {
                if (!pointInsidePoly(pt, outerPath)) return false;
                for (i=0; i<innerpaths.length; i++){
                    if (pointInsidePoly(pt, innerpaths[i])) return false;
                }
                return true;
            }

            var baseItem = placeObject[0],
                targetWidth = baseItem.width * (placeObjectResizeValue / 100),
                targetHeight = baseItem.height * (placeObjectResizeValue / 100),
                stepX = targetWidth + minDistanceToOtherCircles,
                stepY = targetHeight + minDistanceToOtherCircles;

            if (stepX <= 0 || stepY <= 0){
                return alert('Invalid grid step. Check Resize value and Min distance.');
            }

            var gridPoints = [];
            for (y = maxy - (targetHeight / 2); y >= miny + (targetHeight / 2); y -= stepY){
                for (x = minx + (targetWidth / 2); x <= maxx - (targetWidth / 2); x += stepX){
                    var topLeft = [x - (targetWidth / 2), y + (targetHeight / 2)],
                        topRight = [x + (targetWidth / 2), y + (targetHeight / 2)],
                        bottomLeft = [x - (targetWidth / 2), y - (targetHeight / 2)],
                        bottomRight = [x + (targetWidth / 2), y - (targetHeight / 2)];

                    if (
                        isPointInsideWithHoles([x, y]) &&
                        isPointInsideWithHoles(topLeft) &&
                        isPointInsideWithHoles(topRight) &&
                        isPointInsideWithHoles(bottomLeft) &&
                        isPointInsideWithHoles(bottomRight)
                    ){
                        gridPoints.push([x, y]);
                    }
                }
            ...
            progressBarCounter = (gridPoints.length ? progressBar.maxvalue / gridPoints.length : progressBar.maxvalue);
            for (p=0; p<gridPoints.length; p++){
                pt = gridPoints[p];
                var __placeObject = getNode().duplicate(),
                    ...
                    __size = (placeObjectSize === 'width' ? targetWidth : targetHeight),
